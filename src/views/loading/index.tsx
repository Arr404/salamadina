  ...
  return (
    ...
    >
      ...
      <div className="relative flex justify-center items-center overflow-hidden w-full h-full">
        <div
          className="w-40 h-[16.8rem]  overflow-hidden -translate-y-[2rem] relative"
        >
          {[{ number: 0, animation: 'animate-bounce0' },
            { number: 1, animation: 'animate-bounce1' },
            { number: 2, animation: 'animate-bounce2' }].map(
            (item) => (
              <svg
                key={item.number}
                className={`translate-y-[240px] absolute mt-12 w-40 h-40 scale-100 ${item.animation}`}
                style={{
                  top: `${31.704 * item.number}px`,
                  left: `${-19.976 * item.number}px`
                }}
